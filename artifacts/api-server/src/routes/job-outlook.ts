...
const BLS_2024_WAGES: Record<string, number> = {
...
/* ---------- 2024 BLS employment levels (thousands → actual) ---------- */
const BLS_2024_EMPLOYMENT: Record<string, number> = {
...
/* ---------- hardcoded major → SOC crosswalk (fallback when no Supabase) ---------- */
const MAJOR_SOC_MAP: Record<string, { soc: string; title: string }[]> = {
  "computer science": [
...
function findSocsForMajor(major: string): { soc: string; title: string }[] {
...
function getBLSWage(soc: string, title: string): number {
  return BLS_2024_WAGES[soc] ?? getPlaceholderWage(title);
}

function getBLSEmployment(soc: string): number {
  return BLS_2024_EMPLOYMENT[soc] ?? getPlaceholderEmployment(soc);
}

async function fetchBLSData(soc: string, title: string, _blsKey: string) {
  const wage = getBLSWage(soc, title);
  const employment = getBLSEmployment(soc);
...
function getGrowthRate(soc: string): number {
  const prefix = parseInt(soc.slice(0, 2), 10);
  // Tech occupations
  if (soc.startsWith("15-2051")) return 35; // Data Scientists
  if (soc.startsWith("15-1212")) return 33; // Info Security Analysts
  if (soc.startsWith("15-1251")) return 26; // Software Developers
  if (soc.startsWith("29-1171")) return 45; // Nurse Practitioners
  if (soc.startsWith("29-1151")) return 38; // Nurse Anesthetists
  if (soc.startsWith("29-1071")) return 28; // Physician Assistants
  if (soc.startsWith("21-1014")) return 22; // Mental Health Counselors
...
router.get("/job-outlook", async (req, res): Promise<void> => {
  ...
  // Try Supabase first for rich CIP/SOC lookup
  ...
  // Fallback to hardcoded crosswalk
  if (!socList.length) {
  ...
  let jobs: Awaited<ReturnType<typeof fetchBLSData>>[];

  if (blsKey) {
    jobs = await Promise.all(unique.map((s) => fetchBLSData(s.soc, s.title, blsKey)));
  } else {
    // Fallback (no BLS key): use embedded 2024 BLS published data
    jobs = unique.map((s) => ({
      soc: s.soc,
      title: s.title,
