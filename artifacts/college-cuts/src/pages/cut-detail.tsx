...
export default function CutDetail() {
  ...
  function handleCopyLink() {
    const url = `https://college-cuts.com/cuts/${id}`;
        ...
        <div>
          <div className="flex items-center justify-between mb-6 gap-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <a
                href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`${cut?.institution}${cut?.programName ? ` — ${cut?.programName}` : ""} (${cutTypeLabel}) tracked on @collegecutscom`)}&url=${encodeURIComponent(`https://college-cuts.com/cuts/${id}`)}`}
                target="_blank"
              ...
              <a
                href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://college-cuts.com/cuts/${id}`)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center h-8 w-8 rounded-md border border-input bg-background hover:bg-accent transition-colors"
